...
|-----------|--------|----------|-----------|
| Orchestration | Multi-agent (spawns subagents) | Single-agent sessions | GSD-lite targets weaker models in chat apps |
| Context strategy | Fresh windows via Task tool | Checkpoint -> clear -> resume | No programmatic spawning |
| Workflow scope | Project/milestone/phase | Single phase | Incremental work focus |
| Protocol location | .claude/get-shit-done/workflows/ | gsd-lite/workflows/ | Namespace separation |
| Artifact depth | Deep (RESEARCH, PLAN, SUMMARY) | Minimal (STATE, WORK, INBOX) | Weak agent optimization |
| Planning mode | Separate discuss-phase | Embedded moodboard/whiteboard | Simpler mental model |
| Promotion | Automatic after complete | User-controlled | Prevent data loss |

**Key Insight:** GSD-lite is NOT simplified GSD. It's a single-session pattern optimized for weak agents, using GSD's workflow decomposition principles with different constraints.
...
- Contains "GSD-Lite vs OG GSD" comparison table
