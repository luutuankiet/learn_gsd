...
must_haves:
  truths:
    - "User can read any template and understand what to fill in"
    - "User can understand WHY each section exists from inline comments"
    - "Templates work in both MCP (file access) and copy-paste workflows"
    - "User can navigate template directory and find appropriate template"
...
```bash
# Check for systematic ID pattern in LOOPS_TEMPLATE
grep -q 'LOOP-[0-9]\{3\}' .planning/templates/LOOPS_TEMPLATE.md || echo "LOOPS_TEMPLATE.md missing systematic ID pattern (LOOP-NNN)"
...
**7. Requirements Coverage** (Phase 1 goals met?)
   - META-01: Templates heavily commented? (check inline comments)
   - META-02: File-based protocol works via MCP or copy-paste? (check dual instructions)
...
✓ Works in both MCP and copy-paste scenarios
  </how-to-verify>
...
**If issues found:**
Describe specific gaps (e.g., "LOOPS template missing lifecycle documentation", "BOOTLOADER sticky note format unclear", "README workflow diagram missing")
